...
def get_emotion_classifier():
    ...
    if _emotion_classifier is None:
        ...
        # Quantize to int8 to reduce memory
        ...
        print("✅ BERT model loaded and quantized!")
...
def extract_features(text: str, emotions: dict) -> np.ndarray:
    ...
    features = np.array([[
        ...
        joy_score, neutral_score,
        fear_score + sadness_score + anger_score + disgust_score,
        word_count / 100, urgent_count, high_count,
        exclamation_count, caps_ratio, sentence_count, avg_word_length / 10,
    ]])
...
# ── TRAIN XGBOOST (keyword-only, no BERT, runs at startup) ──
def get_xgboost():
    ...
    if _xgb_model is None:
        ...
        for text in training_texts:
            ...
            fear = min(1.0, urgent_hits * 0.3 + high_hits * 0.1)
            sadness = min(1.0, urgent_hits * 0.2 + high_hits * 0.15)
            anger = min(1.0, high_hits * 0.1)
            disgust = min(1.0, urgent_hits * 0.1)
            joy = max(0.0, 1.0 - fear - sadness - anger)
            neutral = max(0.0, 0.5 - fear - sadness)

            emotions = {
                'fear': fear, 'sadness': sadness, 'anger': anger,
                'disgust': disgust, 'joy': joy, 'neutral': neutral
            }
...
# ── INITIALIZE XGBOOST AT STARTUP (lightweight, no BERT) ──
get_xgboost()

# ── MAIN PREDICT FUNCTION ──
def predict_priority(text: str) -> dict:
    # Load BERT only on first actual request (lazy)
    ...
    xgb_model, label_encoder = get_xgboost()
